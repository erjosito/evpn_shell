...
class multicli:
    ...
    def printdebug (self, msg):
        '''
        Prints a debug line either to a file or to stdout, if debug is enabled
        '''
        if self.debug:
            ...
            if self.debugtofile:
                try:
                    with open (self.debugfilename, "a") as configfile:
...
class evpnCli (cmd2.Cmd):
    ...
    def do_debug (self, line):
        '''
        Enable/Disable debug logging either on screen (default) or to a file
        Syntax:     debug  on|off
        Example:    debug on
        '''
        debugstate = line
    ...
    def do_debugtofile (self, line):
        '''
        Enable/Disable debug logging to a file
